...
3. **Pause current spec** (if any) — run full pause workflow (finalize
   state at task boundary, log decisions, set status to paused, run validate)
4. **Load target spec** — read `.specs/<target-id>/SPEC.html`
5. **Activate it** — set its status to `active` in both the
   `<script id="spec-meta">` JSON and `.specs/registry.md`
6. **Resume it** — run the full resume workflow (parse `data-status`,
   identify current phase and first pending task, present summary)
